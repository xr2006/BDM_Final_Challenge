...
PA_FN = '/tmp/bdm/core-places-nyc/*'
#PA_FN = 'core-places-nyc.csv'
pa = sc.textFile(PA_FN, use_unicode=True).cache()
...
def extractPA(partId, part):
  ...
  import csv
  for record in csv.reader(part):
    placekey,naics_code = record[0],record[9]
    yield placekey, naics_code

targeted_stores = pa.mapPartitionsWithIndex(extractPA).filter(lambda x: x[1][:4]=='4451')
targeted_stores.take(5)

storeinfo2 = storeinfo.join(targeted_stores)\
          .mapValues(lambda x:x[0])\
          .filter(lambda x: len(x[1][2])>2)
...
#Step5
results = storeinfo5.mapValues(lambda x:(x[0]*x[1],x[1]))\
...
#spark-submit --executor-cores 5 --num-executors 5 --files nyc_supermarkets.csv,nyc_cbg_centroids.csv BDM_Final_sw5034_EC2.py result_EC2
